...
def test_prepare_data_for_actors_ras_vs_las(tmp_path):
    ...
    slices = (4, 4, 4)
    fa_ras, _, _ = prepare_data_for_actors(dwi_path_ras, bval_path,
                                           bvec_ras_path, tpl_path, slices)
    fa_las, _, _ = prepare_data_for_actors(dwi_path_las, bval_path,
                                           bvec_las_path, tpl_path, slices)

    assert np.allclose(fa_ras, fa_las, atol=1e-4)

    # Verify template orientation variation (LAS template)
    fa_las_tpl, _, _ = prepare_data_for_actors(dwi_path_ras, bval_path,
                                               bvec_ras_path, tpl_las_path,
                                               slices)
    assert np.allclose(fa_ras, fa_las_tpl, atol=1e-4)

    # Verify both DWI and template in LAS
    fa_both_las, _, _ = prepare_data_for_actors(dwi_path_las, bval_path,
                                                bvec_las_path, tpl_las_path,
                                                slices)
    ...
    simg_tpl.to_ras()
    fa_simg, _, _ = prepare_data_for_actors(dwi_path_ras, bval_path,
                                            bvec_ras_path, simg_tpl, slices)
