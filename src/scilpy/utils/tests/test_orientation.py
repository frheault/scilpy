...
def test_parse_voxel_order_invalid_format():
    ...
    # In our implementation, we just take the first 3.
    assert parse_voxel_order("1,2,3,4,5", dimensions=4) == ("R", "A", "S")


def test_parse_voxel_order_4d_valid_numeric():
    """Test parsing of valid 4D numeric voxel order strings."""
    # 4th dimension is ignored, returning only spatial part
    assert parse_voxel_order("1,2,3,4", dimensions=4) == ("R", "A", "S")
    assert parse_voxel_order("-1,2,-3,4", dimensions=4) == ("L", "A", "I")
...
def test_parse_voxel_order_4d_invalid_alpha():
    ...
    with pytest.raises(ValueError,
                       match="Voxel order string must have 3 characters."):
        parse_voxel_order("RASL", dimensions=4)
...
def test_parse_voxel_order_invalid_format_3d():
    ...
    # In our implementation, 4 numbers is accepted but only first 3 used.
    # So we don't raise "4D voxel order is only supported for 4D images."
    # Let's check what it does.
    assert parse_voxel_order("1,2,3,4") == ("R", "A", "S")
